...
- Quick inpaint after inpaint.
- Play notification sound each queue.
- When you focus on load image node, you can change index with keyboard.
...
### Bind key node  
Create a Bind key node.  
When bracket Bind key enabled and you type opening bracket in textarea, bracket has closed and cursor move to inside of brackets.  
...
- Play sound when start generation
```js
sound();
```

- Play sound when end generation
```js
onEnd = () => { sound(); }
```
...
- Setting a callback after image generation
```js
onEnd = (images) => {
  // code here...
}
